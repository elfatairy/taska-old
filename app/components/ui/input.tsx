...
export type InputProps<T extends IconComponent = IconComponent> = React.ComponentProps<"input"> & {
  icon?: T
...
const Input = React.forwardRef<HTMLInputElement, InputProps>((
  ...
  return (
    <div
      className={cn(
        'flex items-center justify-center m-0 p-0 rounded-xl border-1 border-input bg-background px-3 py-0 text-sm transition-colors focus-within:outline-none focus-within:ring-1 focus-within:ring-ring disabled:cursor-not-allowed disabled:opacity-50',
        className
      )}
    >
      ...
      <input
        type={type}
        className={cn(
          'flex flex-1 items-center justify-center h-9 w-full bg-transparent placeholder:text-muted-foreground text-foreground file:border-0 file:bg-transparent file:text-sm file:font-medium focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50',
